...
export default class GoogleDrive extends LightningElement {
    ...
    // ── Wire: Google Drive connection status ──────────────────────────
    @wire(getConnectionStatus)
    wiredStatus({ data, error }) {
        console.log('wiredStatus called');
        if (data) {
    ...
    handleChange(event) {
        this.selectedObject = event.detail.value;
        console.log('Object changed:', this.selectedObject);
        // Object change pe table reset karo
        this.files         = [];
        this.selectedFiles = [];
        this.allSelected   = false;
    ...
    handleCheckbox(event) {
        ...
        this.allSelected = this.selectedFiles.length === this.files.length;
        console.log('Selected files:', JSON.parse(JSON.stringify(this.selectedFiles)));
    }
    ...
    handleSelectAll(event) {
        ...
        console.log('Selected files after select all:', JSON.parse(JSON.stringify(this.selectedFiles)));
    }
    ...
    // ── Upload button click - modal open karo ────────────────────────
    handleUpload() {
        console.log('⬆ Upload clicked');
        console.log('Selected count:', this.selectedFiles.length);
        ...
        console.log('Opening upload modal with files:', this.filesForUpload);

        // Modal component ka reference lo aur open karo
        const modal = this.template.querySelector('c-google-drive-upload-modal');
        if (modal) {
            modal.handleOpenModal();
        } else {
            console.error('Upload modal component not found in template');
        }
    }
